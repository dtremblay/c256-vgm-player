...
; *******************************************************************
; * DAC Stream Commands - TODO: send to FIFO/DAC
; *******************************************************************
DAC_STREAM
            .as
            LDA COMMAND
            CMP #$90
            BNE DAC_SET_STREAM_DATA
            
    DAC_SETUP_CONTROL
            ; skip 4 bytes - ss tt pp cc
            ; stream id ss
            increment_long_addr CURRENT_POSITION
            ; chip type tt 
            increment_long_addr CURRENT_POSITION
            ; port pp
            increment_long_addr CURRENT_POSITION
            ; write command/register cc
            increment_long_addr CURRENT_POSITION
            
    DAC_SET_STREAM_DATA
            CMP #$91
            BNE DAC_STREAM_FREQUENCY
            ; skip 4 bytes - ss dd ll bb
            ; stream id ss
            increment_long_addr CURRENT_POSITION
            ; data bank dd
            increment_long_addr CURRENT_POSITION
    ...
    DAC_STREAM_FREQUENCY
            CMP #$92
            BNE DAC_START_STREAM
            ; skip 5 bytes
            ; stream id ss
            increment_long_addr CURRENT_POSITION
            ; frequency ff ff ff ff
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
    ...
    DAC_START_STREAM
            CMP #$93
            BNE DAC_STOP_STREAM
            ; skip 10 bytes - ss aa aa aa aa mm ll ll ll ll
            ; stream id ss
            increment_long_addr CURRENT_POSITION
            ; data start offset - aa aa aa aa
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
            increment_long_addr CURRENT_POSITION
    ...
    DAC_STOP_STREAM
            CMP #$94
            BNE DAC_START_STREAM_FAST
            ; skip 1 bytes - ss
            ; stream id ss
            increment_long_addr CURRENT_POSITION
    
    DAC_START_STREAM_FAST
            ; skip 10 bytes - ss bb bb ff
            ; stream id ss
            increment_long_addr CURRENT_POSITION
            ; block id bb bb
            increment_long_addr CURRENT_POSITION
